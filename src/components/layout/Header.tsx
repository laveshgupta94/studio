...
export function Header() {
  ...
  const currentAvatar = isAdminPage ? adminAvatar : userAvatar;
  const userName = isAdminPage ? 'Admin User' : 'John Doe';
  const avatarFallback = isAdminPage ? 'AU' : 'JD';
  
  const getHeaderText = () => {
    if (isAdminPage) return 'Admin Portal';
  ...
  return (
    <header className="sticky top-0 z-10 flex h-16 items-center gap-4 border-b bg-background/80 px-4 backdrop-blur-sm sm:px-6 lg:px-8">
      ...
      <h1 className="font-headline text-xl font-semibold">{getHeaderText()}</h1>
